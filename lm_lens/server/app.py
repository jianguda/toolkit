...
import lm_lens.components
from lm_lens.models.tlens_model import TransformerLensTransparentLm
import lm_lens.routes.contributions as contributions
import lm_lens.routes.graph
from lm_lens.models.transparent_lm import TransparentLm
from lm_lens.routes.graph_node import NodeType
from lm_lens.function.repair_interface import RepairInterface, RepairResult
from lm_lens.server.graph_selection import (
    GraphSelection,
    UiGraphEdge,
    UiGraphNode,
)
from lm_lens.server.styles import (
    RenderSettings,
    logits_color_map,
    margins_css,
    string_to_display,
)
from lm_lens.server.utils import (
    B0,
...
from lm_lens.server.monitor import SystemMonitor
...
def cached_build_paths_to_predictions(
    ...
    return lm_lens.routes.graph.build_paths_to_predictions(
...
class App:
    _stateful_model: TransparentLm = None
    ...
    _repair_interface: Optional[RepairInterface] = None
    _repair_result: Optional[RepairResult] = None

    @property
    def stateful_model(self) -> TransparentLm:
        return self._stateful_model

    def __init__(self, config: LmViewerConfig):
        self._config = config
    ...
    def draw_graph(self, contribution_threshold: float) -> Optional[GraphSelection]:
        ...
        return lm_lens.components.contribution_graph(
            model_info,
    ...
    def draw_attn_info(self, edge: UiGraphEdge, container_attention_map) -> Optional[int]:
        ...
        selected_head = lm_lens.components.selector(
    ...
    def draw_ffn_info(self, node: UiGraphNode) -> Optional[int]:
        ...
        selected_neuron = lm_lens.components.selector(
    ...
    def draw_controls(self):
        ...
        with st.sidebar.expander("Model", expanded=True):
            ...
            # Safely get default model index, fallback to 0 if not found
            try:
                default_choice = model_list.index(self._config.default_model) if self._config.default_model else 0
            except ValueError:
                default_choice = 0
        ...
        # Add repair controls
        with st.sidebar.expander("Model Repair", expanded=False):
            self._repair_enabled = st.checkbox("Enable repair", value=False)
            if self._repair_enabled:
                self._repair_approach = st.selectbox(
                    "Repair approach",
                    ["mint", "me-sgd", "me-iter", "me-batch"],
                    index=2,
                    help="Select the repair method: mint (MINT baseline), me-sgd (SGD-based), me-iter (iterative), me-batch (batch)"
                )
                self._repair_epochs = st.slider(
                    "Training epochs",
                    min_value=1,
                    max_value=20,
                    value=10,
                    help="Number of training epochs for repair"
                )
                self._repair_source = st.text_area(
                    "Source prompt",
                    value=self.sentence if self.sentence else "",
                    help="Input prompt for the model"
                )
                self._repair_target = st.text_area(
                    "Target output",
                    value="",
                    help="Desired target output"
                )
                if st.button("Run Repair", type="primary"):
                    if self._repair_source and self._repair_target:
                        self._run_repair()
                    else:
                        st.warning("Please provide both source prompt and target output")
    ...
    def _run_repair(self):
        """Run model repair with the selected approach."""
        try:
            with st.spinner(f"Running repair with {self._repair_approach}..."):
                # Map model name to repair interface format
                repair_model_name = self._get_repair_model_name()
                
                if self._repair_interface is None or repair_model_name != getattr(self._repair_interface, 'model_name', None):
                    self._repair_interface = RepairInterface(
                        model_name=repair_model_name,
                        scale=1,
                        data_name='demo'
                    )
                
                self._repair_result = self._repair_interface.repair(
                    source=self._repair_source,
                    target=self._repair_target,
                    approach=self._repair_approach,
                    epoch_num=self._repair_epochs
                )
    ...
    def _get_repair_model_name(self) -> str:
        """Map UI model name to repair interface model name."""
        # Try to find matching model in MODEL_REGISTRY
        from lm_lens.function.config.shared import MODEL_REGISTRY
        
        # Direct match in values
        if self.model_name in MODEL_REGISTRY.values():
        ...
        # Try to find by partial match in keys
        for key, value in MODEL_REGISTRY.items():
            if key in self.model_name.lower() or self.model_name.lower() in key.lower():
                return value
        
        # Try to find by partial match in values
        for key, value in MODEL_REGISTRY.items():
            if self.model_name.lower() in value.lower() or value.lower() in self.model_name.lower():
                return value
        
        # Default to model_name if no match found
        return self.model_name
    
    def draw_repair_results(self):
        """Draw repair results visualization."""
        if not self._repair_enabled or self._repair_result is None:
            return
        ...
        with col1:
            ...
            st.code(' '.join(self._repair_result.pre_gen_tokens))
        
        with col2:
            st.subheader("After Repair")
            st.code(' '.join(self._repair_result.post_gen_tokens))
        
        with col3:
            st.subheader("Target")
            st.code(' '.join(self._repair_result.target_tokens))
        
        # Calculate accuracy
        pre_match = sum(1 for p, t in zip(self._repair_result.pre_gen_tokens, self._repair_result.target_tokens) if p == t)
        post_match = sum(1 for p, t in zip(self._repair_result.post_gen_tokens, self._repair_result.target_tokens) if p == t)
        total = len(self._repair_result.target_tokens)
        
        st.metric("Token Accuracy (Before)", f"{pre_match}/{total} ({pre_match/total*100:.1f}%)")
        st.metric("Token Accuracy (After)", f"{post_match}/{total} ({post_match/total*100:.1f}%)")
        
        # Show edited neurons if available (for MINT)
        if self._repair_result.edited_neurons is not None:
            st.subheader("Edited Neurons")
            total_neurons = sum(len(neurons) for neurons in self._repair_result.edited_neurons)
            st.write(f"Total neurons edited: {total_neurons}")
            
            if self._repair_result.nums_edit:
                st.write("Neurons edited per token:")
                edit_data = {
                    "Token Index": list(range(len(self._repair_result.nums_edit))),
                    "Neurons Edited": self._repair_result.nums_edit
                }
                st.bar_chart(edit_data, x="Token Index", y="Neurons Edited")
    ...
    def run(self):
        ...
        # Draw repair results if available
        if self._repair_enabled:
            self.draw_repair_results()
