...
from lm_lens.models.transparent_lm import ModelInfo, TransparentLm
...
def load_hooked_transformer(
    ...
    # Try to get official model name, but handle ValueError if model is not in transformer_lens registry
    try:
        supported_model_name = get_official_model_name(supported_model_name)
    except ValueError:
        # Model not in transformer_lens registry
        # For Qwen2.5-Coder models, try mapping to Qwen2 models that transformer_lens supports
        # Or use the model_name directly if it's a valid HuggingFace model
        if "Qwen2.5-Coder" in model_name:
            # Map Qwen2.5-Coder to Qwen2 (transformer_lens supports Qwen2 but not Qwen2.5-Coder)
            # Try to find a similar Qwen2 model
            if "0.5B" in model_name:
                supported_model_name = "Qwen/Qwen2-0.5B"
            elif "1.5B" in model_name:
                supported_model_name = "Qwen/Qwen2-1.5B"
            elif "3B" in model_name:
                # transformer_lens doesn't have Qwen2-3B, use model_name directly
                supported_model_name = model_name
            else:
                supported_model_name = model_name
        else:
            # For other unsupported models, use the model_name directly
            # transformer_lens can still load them from HuggingFace
            supported_model_name = model_name
    
    # Register the alias if needed
    if supported_model_name not in MODEL_ALIASES:
    ...
    # Load the model using the original HuggingFace name
    # transformer_lens will use supported_model_name for architecture detection
...
# in mutexes.
class TransformerLensTransparentLm(TransparentLm):
    """
    Implementation of Transparent LM based on transformer lens.
